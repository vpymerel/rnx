...
sshpass -p "*************" \
scp vmerel@curnagl.dcsr.unil.ch:/work/FAC/FBM/DEE/tschwand/myrmecophilus/WGS/SR2Female/Calling/All.counts ~/Desktop
```
...
##Htz
Htz <- read.delim("~/Desktop/All.counts", header=FALSE)
...
All %>% filter(sp=="Mms") %>%
  ...
  scale_colour_manual(values=c("black","#D46A6A","#407F7F","#55AA55"))
  ...
  theme(legend.position = "none",
    ...
    scale_x_continuous(breaks=c(0,50,100,150),
                       labels=c("0Mb",
                                ...
                                "150Mb"))+
    expand_limits(y = 0.04)+#for Htz (mfs)
  scale_colour_manual(values=c("black","#D46A6A","black","black"))
...
ToKeep <- paste("scaffold_",1:2, sep="")
df <- Htz %>%
  filter(cs %in% ToKeep)
      ...
      ifelse(cs=="scaffold_1" & start < 127000000, "X0",
             ifelse(cs=="scaffold_1" & start >= 127000000 & start < 140250000, "Xa",
                    ...
                    ifelse(cs=="scaffold_1" & start >= 144000000 & start < 157000000, "Xb",
                           ifelse(cs=="scaffold_1" & start >= 157000000 & end < 170024705, "PAR","Auto"))))))


df$categ=factor(df$categ, levels=c("X0","Xa","Xb","PAR","Auto")) #,"XYc"
df$sp=factor(df$sp, levels=c("Mms","Mun","Mfg"))


df %>%
  filter(categ!="X0"& sex=="m" & sp=="Mms") %>%
  ggplot(aes(x=categ,
             y=htz,
             color=categ))+
  geom_boxplot() +
...
ggsave("/home/vmerel/mse/Supplementary/Figures/Htz_Mfg.svg",
       units="cm",
       width=10,
       height=12)
...
#Mms stats
median(df$htz[df$categ=="Auto" & df$sp=="Mms" & df$sex=="m"])
...
wilcox.test(df$htz[df$categ=="PAR" & df$sp=="Mun" & df$sex=="m" & df$Mapping=="minimap2"], df$htz[df$categ=="PAR" & df$sp=="Mms" & df$sex=="m" & df$Mapping=="minimap2"])
      ...
      ifelse(cs=="scaffold_1" & start < 127000000, "0-127Mb (X0)",
             ifelse(cs=="scaffold_1" & start >= 127000000 & start < 140250000, "127-144Mb (Xa)",
                    ...
                    ifelse(cs=="scaffold_1" & start >= 144000000 & start < 157000000, "144-157Mb (Xb)",
                           ifelse(cs=="scaffold_1" & start >= 157000000 & end < 170024705, "157-170Mb (PAR)","Auto."))))))

Cov$categ=factor(Cov$categ, levels=c("0-127Mb (X0)","127-144Mb (Xa)","144-157Mb (Xb)","157-170Mb (PAR)","Auto."))
...
ggsave("~/mse/Supplementary/Figures/Mun_cov.svg",
       units = "cm",
       width = 18,
       height = 20.8)
...
ggsave("~/mse/Supplementary/Figures/Mfg_cov.svg",
       units = "cm",
       width = 18,
       height = 20.8)
